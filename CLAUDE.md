...
## ⚠️ AIアシスタント向け: 必須の作業ワークフロー (agy連携)
**機能の実装が完了した際は、必ずあなた自身でターミナルから以下のコマンドを実行し、Gemini（agy）のコードレビューとセキュリティチェックを受けてください。**
`agy "直近の実装内容について、要件・ドキュメント通りか確認し、セキュリティの観点からレビューしてください"`
**コマンドの標準出力からagyのレビュー結果を読み取り、修正指示や脆弱性の指摘があった場合は、その指示に従って自律的に修正を行ってください。すべての指摘が解決し、agyから「問題なし」と評価されるまでこのサイクルを繰り返した上で、ユーザーに完了報告を行ってください。**
**【無限ループ防止（サーキットブレーカー）】**
**レビューと再修正のサイクルが「最大5回」続いても解決しない場合は、設計や指示自体に根本的な問題がある可能性があります。無限ループを防ぐため直ちに修正作業を中断し、`TODO.md` に「【要確認】〇〇の機能実装でagyのレビューが通らないため指示の見直しが必要」というタスクを追記してユーザーへ報告してください。**
