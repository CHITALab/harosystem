...
## ⚠️ 必須開発ワークフロー（要件 → 承認 → 実装 → 動作確認 → 監査 → 受け入れ）

> **なぜこのフローなのか**: 「全く使えない実装が監査を通る」事故を防ぐため。原因は
> **仕様を書いた本人(Gemini)が、自分の書いた仕様を基準に、アプリを動かさずに監査する「閉じたループ」**だった。
> 仕様と実装が内部的に一致していても、ユーザーの意図とズレていたり実行時に壊れていたりすると「使えない実装」が通ってしまう。
> これを断つため、**①ユーザー承認**と**②実動作検証**を関所として挟む。
...
|---|---|---|------|
| 1 | 要望（ざっくりでよい） | ユーザー | |
| 2 | **受け入れ条件**の起票（実装手順ではなく "観測可能な完了条件"） | Gemini | |
| 3 | **受け入れ条件の承認** | ユーザー | ★承認が出るまで着手禁止 |
| 4 | 実装 | Claude Code | |
| 5 | **動作確認**（アプリを実際に動かし受け入れ条件を1件ずつ検証＋回帰テスト） | Claude Code | ★全条件クリアで次へ |
| 6 | agy 監査（受け入れ条件と実挙動を基準にレビュー → 自律修正 最大5回） | Gemini | |
| 7 | **受け入れ確認**（ユーザーが実際に触ってOK） | ユーザー | ★ここで初めて完了 |
| 8 | `[x]` 化 → `CHANGELOG.md` へ退避、機能を使い始める | Claude Code | |

### 3つの原則

1. **受け入れ条件が書けないタスクは着手しない。** 曖昧な要望をここで強制的に言語化する関所にする。実装メモは"参考"であって、完了条件を縛ってはならない。
2. **実装完了 ≠ 動作する。** ステップ5の動作確認を全項目クリアするまで監査(6)に進まない。
3. **監査PASS ≠ 完了。** ステップ7でユーザーが実際に使えて初めて完了とする。

### TODO エントリ・テンプレート（必須フォーマット）

`TODO.md` の各タスクは以下の形で起票する。**「受け入れ条件」が空のまま着手してはならない。**

```markdown
### [機能名]
**ユーザーストーリー**: <誰として> <何がしたいか> <なぜ>
**受け入れ条件（Acceptance Criteria / 観測可能・検証可能）**:
- [ ] 〜画面を開くと、〜が表示される
- [ ] 〜をD&Dすると〜へ移動し、リロード後も維持される
- [ ] （異常系）〜が無い場合は〜と表示される
**スコープ外**: <今回やらないこと>
**実装メモ（任意・参考）**: <技術的ヒント。受け入れ条件を縛らない>
**動作確認結果（実装後に記入）**: <各受け入れ条件の ✅/❌ と確認方法>
```

### ステップ5: 動作確認のやり方

- 受け入れ条件を **1項目ずつ実際のアプリで再現** し、✅/❌ をエントリの「動作確認結果」に記録する（DBへ投入して画面確認、または API を叩いて検証）。
- **データ整合性に関わる変更**（マイグレーション・取得クエリ・FK 変更等）は **最低1本の回帰テスト**（`pytest` / `jest`）を残す。
- 全項目 ✅ になってから監査(6)へ進む。❌ が残るうちは監査に出さない。

### ステップ6: agy 監査

動作確認をクリアしたら、ターミナルから以下を実行し Gemini（agy）の監査を受ける。**監査の基準は「受け入れ条件」と「実際の挙動」**である点に注意。

```bash
agy --model gemini-3.1-pro "直近の実装を、以下の観点で厳格に監査してください。最優先は【0】です。
0. 受け入れ条件（TODOエントリ記載）を実際の挙動として満たしているか／実行時に壊れる箇所はないか（データ消失・未表示・例外）
1. 可読性の高さ、適切な命名が行われているか
2. 設計上の欠陥、適切な責務分離ができているか
3. 保守性、拡張性、テストしやすい構造になっているか
4. パフォーマンス上の問題、およびセキュリティ上の問題がないか
5. バグの温床、または「動くがプロなら書かない」素人っぽい実装がないか
6. アンチパターン、使用言語・フレームワークの慣習違反がないか
7. コメントの質（設計意図やWhyが適切に書かれているか）
指摘があればMarkdownでリストアップし、なければ『問題なし』と出力してください。"
```

**【フォールバック】** `agy` が Rate Limit / タイムアウトで失敗したら `--model gemini-3.1-flash` 等、次に性能の高いモデルに切り替えて再実行する。

**【自律修正】** 指摘があれば自律的に修正し再監査。「問題なし」になるまで繰り返す。**ただし受け入れ条件に関わる修正をしたら、ステップ5の動作確認をやり直してから再監査する。**

**【サーキットブレーカー】** 監査と再修正のサイクルが **最大5回** で解決しない場合は、設計や指示自体に根本問題がある可能性が高い。直ちに中断し、`TODO.md` に「【要確認】〇〇でagy監査が通らないため指示の見直しが必要」を追記してユーザーへ報告する。

### ステップ8: 完了後の TODO 衛生

- 完了した `[x]` 項目（実装ノート含む）は **`CHANGELOG.md` へ移動**し、`TODO.md` は **未着手バックログだけ**に保つ。源泉(source of truth)の信号対雑音比を高く維持し、AI が要件を読み違える余地を減らす。
...
│           │   ├── login/          # ログイン画面 (JWT 認証)
│           │   ├── notes/          # ノート管理ページ (/notes)
│           │   ├── board/          # アクティブスプリントのボード (/board)
│           │   └── backlog/        # バックログ & スプリント計画 (/backlog)
│           └── ui/                 # 再利用可能な UI パーツ
│               ├── badge.component.ts
│               ├── button.component.ts
...
└── backend/                # ===== FastAPI REST API =====
    ...
    └── app/
        ...
        ├── recurrence.py   # 繰り返し予定 (RRULE) の展開ロジック
        └── routers/        # API ルートハンドラ
            ├── events.py
            ├── tasks.py
            ├── labels.py
...
- **タスク時刻**: `start_at` / `end_at`（両方指定でスケジュール / 両方 null で未スケジュール=バックログ。end > start）
- **コンテンツ**: 最大 100,000 文字
...
| スプリント | `/api/sprints` | GET, POST |
| スプリント (個別) | `/api/sprints/{id}` | PUT, DELETE |
| スプリント開始/完了 | `/api/sprints/{id}/start`, `/api/sprints/{id}/complete` | POST |
...
Sprint ────< Task   (sprint_id, ON DELETE SET NULL / null = バックログプール)
...
- Sprint → Task: `sprint_id` FK (ON DELETE SET NULL — スプリント削除でタスクはプールへ)
- Label → Sprint: `label_id` FK (ON DELETE SET NULL — ラベル単位でスプリントを分離。null=未分類)
- Feed → FeedEvent: `feed_id` FK (ON DELETE CASCADE)
- Task.status (backlog/todo/in_progress/done) ⇔ done を相互同期。sprint_id=null ⇒ status=backlog
...
| タスク `start_at`/`end_at` | 両方指定 or 両方 null。指定時は end > start |
